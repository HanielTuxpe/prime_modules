...
const CardCheckout = ({ precio, isOffline }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!stripe || !elements || isOffline) {
      toast.error('❌ Los pagos en línea requieren conexión a internet.');
      return;
    }
    ...
    <Paper elevation={3} sx={{ backgroundColor: '#8A1538', p: 4, mt: 2 }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Button
          ...
          variant="contained"
          fullWidth
          disabled={!stripe || loading || isOffline}
          sx={{
        ...
        >
          {loading ? 'Procesando...' : isOffline ? 'No disponible sin conexión' : 'Confirmar Pago'}
        </Button>
      </form>
    </Paper>
  );
...
const Payments = () => {
  ...
  const handleDownloadFormato = (datosEstudiante, concepto, precio) => {
    ...
    doc.text(
      'Realice su pago en los medios oficiales. Guarde este formato como comprobante preliminar.',
      20,
    ...
    <Box sx={{ p: 4, minHeight: '100vh' }}>
      ...
      {isOffline && (
        <Alert severity="warning" sx={{ mb: 2, borderRadius: '12px' }}>
          ⚠️ Estás sin conexión. Los pagos en línea no están disponibles, pero puedes generar el formato de pago.
        </Alert>
      )}
          ...
          <Paper elevation={3} sx={{ p: 4 }}>
            ...
            {tipoPago === 'formato' && (
              <Box sx={{ mt: 2 }}>
                ...
                >
                  {studentData ? 'Descargar formato de pago' : 'Cargando datos...'}
                </Button>
              </Box>
            )}
            ...
            {tipoPago === 'tarjeta' && (
              <Box sx={{ mt: 2 }}>
                <Elements stripe={stripePromise}>
                  <CardCheckout precio={precio} isOffline={isOffline} />
                </Elements>
              </Box>
            )}
          </Paper>
