...
import { CssBaseline, ThemeProvider, Box, Container, Alert } from '@mui/material';
...
const App = () => {
  ...
  // precarga APIS que deben estar cacheadas
  useEffect(() => {
    if (navigator.onLine) {
  ...
  const handleLogin = (tipoUsuario) => setUsuario(tipoUsuario);
  ...
  const Layout = ({ children }) => (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100%' }}>
      ...
      {offline && (
        <Alert severity="warning" sx={{ width: "100%", borderRadius: 0, textAlign: "center" }}>
          No tienes conexión a Internet - usando datos guardados.
        </Alert>
      )}

      <Container disableGutters maxWidth={false} sx={{ mt: 1, flexGrow: 1, width: '100%' }}>
        {children}
      </Container>
  ...
  const RutaPrivada = ({ children, tipoPermitido }) => {
    if (!usuario) return <Navigate to="/Publico/login" replace />;
    if (usuario !== tipoPermitido) return <Navigate to="/Publico/Error400" replace />;
    return children;
  };

  RutaPrivada.propTypes = { children: PropTypes.node.isRequired, tipoPermitido: PropTypes.string.isRequired };
      ...
      <Router>
        <Layout key={usuario ? 'authenticated' : 'guest'}>
          <Routes>
            ...
            {/* Student */}
            <Route path="/Estudiante/" element={<RutaPrivada tipoPermitido="Estudiante"><Calendar /></RutaPrivada>} />
            <Route path="/Estudiante/Calificaciones" element={<RutaPrivada tipoPermitido="Estudiante"><Calificaciones /></RutaPrivada>} />
            <Route path="/Estudiante/Historial" element={<RutaPrivada tipoPermitido="Estudiante"><Historial /></RutaPrivada>} />
            <Route path="/Estudiante/Rendimiento" element={<RutaPrivada tipoPermitido="Estudiante"><Rendimiento /></RutaPrivada>} />
            <Route path="/Estudiante/Predicciones" element={<RutaPrivada tipoPermitido="Estudiante"><Predicciones /></RutaPrivada>} />
            <Route path="/Estudiante/Perfil" element={<RutaPrivada tipoPermitido="Estudiante"><Profile /></RutaPrivada>} />
            <Route path="/Estudiante/Pagos" element={<RutaPrivada tipoPermitido="Estudiante"><Payments /></RutaPrivada>} />

            {/* Teacher */}
            <Route path="/Docente/" element={<RutaPrivada tipoPermitido="Docente"><HorarioDocente /></RutaPrivada>} />
            <Route path="/Docente/GrupoAsesorado" element={<RutaPrivada tipoPermitido="Docente"><GrupoAccesorado /></RutaPrivada>} />
            <Route path="/Docente/GrupoAsesorado/Riesgo" element={<RutaPrivada tipoPermitido="Docente"><GrupoAsesoradoRiesgo /></RutaPrivada>} />
            <Route path="/Docente/MateriasImpartidas" element={<RutaPrivada tipoPermitido="Docente"><MateriasImpartidasView /></RutaPrivada>} />
            <Route path="/Docente/MateriasImpartidas/Riesgo" element={<RutaPrivada tipoPermitido="Docente"><MateriasImpartidasAlumRiesgo /></RutaPrivada>} />
